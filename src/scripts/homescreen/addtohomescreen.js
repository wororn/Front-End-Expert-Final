...
(function (window, document) {
  ...
  const _eventListener = 'addEventListener' in window;
  ...
  const _reSmartURL = /\/ath(\/)?$/;
  const _reQueryString = /([\?&]ath=[^&]*$|&ath=[^&]*(&))/;
  ...
  ath.Class.prototype = {
    ...
    show(force) {
      ...
      const now = Date.now();
      const { lastDisplayTime } = this.session;
      ...
      // add the action icon
      message = `<p>${message.replace(/%icon(?:\[([^\]]+)\])?/gi, (matches, group1) => `<span class="ath-action-icon">${group1 || 'icon'}</span>`)}</p>`;
    ...
    updateViewport() {
      ...
      const screenWidth = ath.OS == 'ios' ? this.orientation == 'portrait' ? screen.width : screen.height : screen.width;
